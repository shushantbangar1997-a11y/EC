...
### Live Chat (Task 24)
- **Customer**: `src/components/ChatWidget.jsx` — floating bubble bottom-right (above WhatsApp), expand panel with optional name/email gate for guests, image attach (paperclip), heartbeat every 10s with current page url, single-check ✓ on send / double-check ✓✓ in green when admin reads. Hidden on `/admin/*` and `/auth/*`. Persists `ec_chat_session_id`, `ec_chat_token`, `ec_chat_name`, `ec_chat_email`, `ec_chat_open` in localStorage.
- **Admin**: `src/pages/admin/LiveChat.jsx` — two-pane console (left: visitor list w/ online/all filter + search by name/email/phone/page/preview, right: thread + composer with explicit "Start Chat" CTA when no messages exist). Composer supports text, drag-drop, and paste-from-clipboard image upload. Visitor messages get a green double-check the moment the admin reads them. Sidebar nav group "Chat" with badge = active visitors.
- **Server**: `server/index.js` wraps Express in an `http.Server` + Socket.IO. Chat helpers (`ensureChatStores`, `upsertChatSession`, `appendChatMessage`, `markChatMessagesRead`, etc.) live in `server/db.js`. Events: `visitor:hello/heartbeat/typing/message`, `admin:hello (JWT)/join-session/leave-session/typing/message/end-session/mark-read`. Server emits `chat:identity` (mints `session_id` + signed JWT chat_token), `chat:hello`, `chat:message`, `chat:transcript`, `chat:typing`, `chat:read` (read-receipt delta), `chat:ended` (with `reason: 'offline_timeout'` on auto-archive), `chat:snapshot`, `chat:session-update`, `chat:auth-error`.
- **Lifecycle**: `PRESENCE_CLEANUP_MS = 5s` removes a visitor from the live presence map on disconnect; `OFFLINE_AUTO_END_MS = 5min` then auto-archives the session (`status: 'ended'`). Both timers cancel on reconnect via the `autoEndTimers` map.
- **REST**: `POST /api/chat/upload` (multer, 5 MB images only — JPG/PNG/WEBP/GIF, **memory storage** so unauthenticated requests never write to disk; requires admin Bearer token OR matching `chat_token` + `session_id`; per-session rate limit 10/min); `GET /api/chat/sessions` (admin/operator, returns `online_count`); `GET /api/chat/sessions/:id/messages?chat_token=…` (visitor must present token, admin can fetch any).
...
- **Wiring**: `src/App.jsx` mounts `<ChatWidget />` and lazy-loads `/admin/live-chat`. `AdminPortalLayout.jsx` opens its own admin socket so the Live Chat badge updates the instant a visitor lands/leaves (the existing 5s poll stays as a safety net). `vite.config.js` proxies `/socket.io` (with `ws:true`) and `/uploads`.
